...
const Courses = () => {
    ...
    const clearAllFilters = () => { setSearchTerm(""); setActiveFilter("All"); setFilteredCourses(coursesData); setCurrentPage(1); };
                            ...
                            <div className="flex flex-col md:flex-row gap-4 mb-6">
                                <div className="flex-1 relative">
                                    <Search
                                        ...
                                        size={18}
                                    />
                                    <Input type="text" placeholder="Search courses..." className="pl-10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                </div>
                                <Button variant="outline" className="gap-2" onClick={() => setIsFiltering(!isFiltering)}>
                                    <Filter size={18} />
                                    Filter Courses
                                </Button>
                            </div>
                                ...
                                <div className="glass-card p-4 mb-6">
                                    <div className="flex justify-between items-center">
                                        ...
                                        {activeFilter && activeFilter !== "All" && (
                                            <Button variant="ghost" size="sm" className="text-xs text-tutor-error hover:text-tutor-error/80" onClick={() => filterByLevel("All")}>
                                                <X size={14} />
                                                Clear Level Filter
                                            </Button>
                                        )}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {[
                                            ...
                                            <button key={level} className={`px-3 py-1 text-sm rounded-full transition-colors ${activeFilter === level ? "bg-[#0A84FF] text-white" : "bg-[#0A84FF]/10 text-[#1D1D1F] hover:bg-[#0A84FF]/20"}`} onClick={() => filterByLevel(level)}>
                                                {level}
                                            </button>
                                        ))}
                                    </div>
                        ...
                        {filteredCourses.length > 0 && (
                            <Pagination className="mt-12">
                                <PaginationContent>
                                    <PaginationItem>
                                        <PaginationPrevious
                                            className={currentPage === 1 ? "pointer-events-none opacity-50" : ""} onClick={() => handlePageChange(currentPage - 1)} />
                                    </PaginationItem>
                                    ...
                                    <PaginationItem>
                                        <PaginationNext className={currentPage === totalPages ? "pointer-events-none opacity-50" : ""} onClick={() => handlePageChange(currentPage + 1)} />
                                    </PaginationItem>
                                </PaginationContent>
                            </Pagination>
                        )}
