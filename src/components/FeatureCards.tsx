...
import { Cpu, Shield, Clock, Users, TrendingUp, MessageSquare, } from "lucide-react";

interface FeatureCardProps { icon: JSX.Element; title: string; description: string; delay: number; className?: string; }
...
const FeatureCard = ({ icon, title, description, delay, className, }: FeatureCardProps) => {
    ...
    return (
        <div ref={cardRef} className={`glass-card p-6 opacity-0 translate-y-8 transition-all duration-700 ${className}`} >
            ...
            <div className="max-w-6xl mx-auto">
                <div className="text-center mb-16">
                    ...
                    <p className="max-w-2xl mx-auto text-[#86868b]">
                        Our platform combines cutting-edge AI technology with blockchain security create a learning environment that's both innovative and trustworthy.
                    </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {features.map((feature, index) => (
                        <FeatureCard key={index} icon={feature.icon} title={feature.title} description={feature.description} delay={index * 100} />
                    ))}
                </div>
            </div>
        </section>
